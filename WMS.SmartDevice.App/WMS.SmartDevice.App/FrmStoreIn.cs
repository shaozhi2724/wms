    ...
    public partial class FrmStoreIn : Form
    {
        ...
        public void BindStoreInRecords()
        {
            ...
            using (SqlConnection conn = new SqlConnection(Program.ConnectionString))
            {
                conn.Open();
                _models =
                    conn.Query<StoreBillInViewModel>("SELECT DISTINCT SB.StoreID,ISNULL(P.ProductName,'') AS ProductName,StoreCode,StoreTypeName,DealerName FROM BUS_StoreBill AS SB INNER JOIN WMS_StoreInDetail AS SBD ON SBD.StoreID = SB.StoreID LEFT JOIN dbo.BFI_Product AS P ON P.ProductID = SBD.ProductID WHERE IsBillIn=1 AND ISNULL(SB.BillStyle,'正常')<>'直调' AND StoreTypeID<>3 AND CAST(ISNULL(SB.Reservation1,'0') AS INT)<3")
                        .ToList();
